    ...
    describe(`Test ${user_id} from ` + emu.messenger_type, function(){
        ...
        describe("Unknown question", function(){
            it("will trigger escalation skill", function(){
                ...
                const question = `Where is the restroom?`;
                ...
                return emu.send(event).then(async function(context){
                    ...
                    context.previous.message[0].message.contents.body.contents[2].text.should.equal(`お手洗いはどこですか？`);
                    ...
                    JSON.parse(context.previous.message[0].message.contents.footer.contents[0].action.data).intent.parameters.question.should.equal(`お手洗いはどこですか？`);
